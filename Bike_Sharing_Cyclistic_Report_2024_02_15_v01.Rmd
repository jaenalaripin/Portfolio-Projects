---
title: "Analysis of Users' Behavior"
...
### 2. Data Source
The Cyclistic company has all the data about every transaction. We use the data from the company for analysis. The data are divided into 4 quarters per year. In this analysis, we only include the transaction report in 2019, which spread to 4 separate data sets (Q1, Q2, Q3, and Q4).
...
```{r heatmap of transactions, echo=FALSE}
...
```{r distributions of users age, echo = FALSE}
...
bike2019durationday %>% ggplot(aes(x = days, y = average, fill = usertype)) + geom_col(position = "dodge") + 
  theme_minimal() +
  theme(
    text = element_text(family = "Courier New"),
    title = element_text(face = "bold")) + 
