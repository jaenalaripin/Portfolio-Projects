...
The Cyclistic company has all the data about every transaction. We use
the data from the company for analysis. The data are divided into 4
quarters per year. In this analysis, we only include the transaction
report in 2019, which spread to 4 separate data sets (Q1, Q2, Q3, and
Q4).
...
``` r
sum(duplicated(bike2019$trip_id))                #trip_id should be unique, so no duplicates are allowed in this column
```

    ## [1] 0

``` r
bike2019 %>% distinct(usertype)                  #checking values in usertype column
```
...
``` r
bike2019 %>% distinct(gender)                    #checking values in gender column
```
...
| start_time | usertype      | days | total |
|:-----------|:--------------|:-----|------:|
| 00:00:01   | Annual Member | Thu  |     1 |
| 00:00:01   | Annual Member | Fri  |     1 |
| 00:00:01   | Annual Member | Sat  |     1 |
| 00:00:01   | Casual        | Mon  |     1 |
| 00:00:01   | Casual        | Fri  |     1 |
| 00:00:02   | Annual Member | Sun  |     1 |
...
![](Bike_Sharing_Cyclistic_Report_2024_02_15_v01_files/figure-gfm/heatmap%20of%20Time-1.png)<!-- -->

    ## Warning: Removed 1 rows containing missing values (`geom_tile()`).

![](Bike_Sharing_Cyclistic_Report_2024_02_15_v01_files/figure-gfm/heatmap%20of%20Time-2.png)<!-- -->
