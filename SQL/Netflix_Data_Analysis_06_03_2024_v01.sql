...
SELECT *, 
	   ROW_NUMBER() OVER(PARTITION BY type, title, director, cast, country, 
							   ...
							   description ORDER BY title) as count
FROM NetflixMovies.dbo.netflix_titles) 
...
----------------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------- CREATE VIEW FOR TABLEAU VIZUALIZATIONS-----------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------
DROP VIEW IF EXISTS netflix
CREATE VIEW netflix AS
WITH 
delimited AS (
SELECT show_id, type, title, REPLACE(director, ', ', ',') AS director, 
REPLACE(cast, ', ', ',') as cast, REPLACE(country, ', ', ',') AS country, 
date_added, REPLACE(listed_in, ', ', ',') AS genre, rating, 
duration_minutes,
seasons
FROM NetflixMovies.dbo.netflix_titles
),
