...
\* [1.
Ask](#cell0)<br> \* [2. Prepare](#cell1)<br> \* [3. Process](#cell2)<br>
...
* Three dataset were wide tables, we excluded these datasets from analysis because there were other datasets in long tables that cotained the same values.
...
cleaning, analyzing, visualizing, and reporting. We imported the datasets to RStudio using `read_csv` function
from `readr` package.
...
The datasets were imported from external source and we found some issues like:
...
* we converted the data type from string to date, and
...
We use the codes below to check if the datasets contained duplicates.
...
We found outliers in the *dailyactivity* table, there was a column called *SedentaryMinutes*. Sedentary minutes meant the amount of time (in minutes) the users use for not doing anything (sitting
still, laying down, or sleeping). In that column, we found
values returned *1440*. It meant the users did nothing for *1440* minutes
or 24 hours.
...
These were the tables we were trying to create:
...
With this, we could count the numbers of users in each group.
...
By performing this part of analysis, we can determined the days where the users mainly wore the devices. 
...
* METs values in the **minutesactivity** and
* intensity values in the **hourlyactivity**.

We wanted to perform analysis based on all variables, so we merged the
tables and summarized all variables grouped by the *ActivityDate*.
...
analysis, which included the values of:
...
for the metabolic equivalent of task. One MET is the amount of energy used while sitting quietly. Physical activities may be rated using METs to indicate their intensity. For example, reading may use about 1.3 METs
while running may use 8-9 METs. METs can also be translated into light, moderate, and vigorous intensities of exercise.
...
We also determined whether the users already achieved their fitness target or not. As a baseline, the NHS recommends adults do at least 150 minutes of
...
We splitted the users into two groups based on that theory.
``` r
...
This chart visualized the numbers of users who used the device actively, moderately, or lightly.

![](Bellabeat_2024_02_16_v01_files/figure-gfm/frequency%20level%20chart-1.png)<!-- -->

## 5.2. People Forgot to Wear Their Devices

Sometimes, people had the smart devices on but forgot to wear them.
...
confidence levels were just too low.

We could rely on those variables to maintain our fitness level, but we relied more on MET. We know
...
activities in higher MET and burned more calories. People who lift weigth for less than an
...
you the fitness level of the users.
...
workouts. This explained why sedentary intensities has highest numbers among the others, because most of the people used their time mainly for work
(7-9 hours) and rest, while the other intensities like
...
- Moderate intensity included walking briskly, vacuuming, or raking
  leaves.
- Vigorous intensity included walking very quickly, running, taking anaerobics class, or shoveling snow

they did them ocassionally.

### 5.5.2. Heatmap Of Average MET Per User Over The Month

We could see the clearer result of how much average MET values that each user had daily
...
higher the MET, the higher the metabolic rate would be and the more calories would be burned.

### 5.5.3. Fitness Achievement Status
...
tiles with oranges to show that the users achieved the target, and with
yellows to show that the users should do more exercise.
...
We could say people who used this smart devices concerned about their health
